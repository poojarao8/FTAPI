...
EXPORT void set_default_front_options(
	...
	Front     *front)
{
	char	s[Gets_BUF_SIZE]; /* Gets_BUF_SIZE = 513 magic number? */
	...
	if (restart_io_type(init) != NULL)
	    read_print_front_options(init,front);
	/* call f_read_print_front_options */

	set_redistribution_defaults(init);
	/* call f_set_redistribution_defaults */

	    /* Choose Constant to Increase/Decrease Time-Steps */

	set_front_time_step_control_default(init);
	/* call f_set_front_time_step_control_default */
	...
	{
	    normal3d_method(init) = WLSP_NORMAL;
	    /* Set the normal and curvature calculation method */
	    if (restart_io_type(init) != NULL)
	        normal3d_method(init) = NORMAL_METHOD_FROM_RESTART;
	}
	...
	Front	      *front)
{
	...
	Init_redistribution(init,front); /*call f_init_redistribute */
	...
	INIT_DATA *init)
{
	...
	maximum_scaled_triangle_side_length(init) = 1.3;
        /*
	minimum_angle_at_triangle_vertex(init,GENERAL_WAVE) = radians(30.0);
	minimum_angle_at_triangle_vertex(init,VECTOR_WAVE) = radians(30.0);
	maximum_scaled_triangle_side_length(init) = 1.0;
        */
	...
	F_BASIC_DATA *ft_basic)
{
	...
	{
	        if (ft_basic->coord_system == CYLINDRICAL_REMAP)
		    front->coordinate = 'c';
		else if(ft_basic->coord_system == SPHERICAL_REMAP)
		    front->coordinate = 's';
		else
		    front->coordinate = 'r';
		...
		front->out_name = ft_basic->out_name;
        	...
        	{
            		subdomains(init_data(&Init))[i] = 1;
            		buffer_zones(init_data(&Init))[i] = 
				ft_basic->buffer_size[i];
#if defined(__MPI__)
		...
		/* set global variable size_of_intfc_state */
		set_size_of_intfc_state(front->sizest);

		/* most function pointers in INTERFACE set here */
        	f_set_interface_hooks(dim,init_data(&Init));
		...
		if (ft_basic->RestartRun)
		{
		    ...
		    FILE *rfile;
		    comp_grid.dim = dim;
                    if(ft_basic->RegridRestart)
                        front->regrid_restart = YES;
                    for (i = 0; i < dim; ++i)
                        front->gmax[i] = ft_basic->gmax[i];
                    for (i = 0; i < dim; ++i)
                        front->gmax_regrid[i] = ft_basic->gmax_regrid[i];

		    set_remap_and_rect_grid(ft_basic->L,ft_basic->U,
			    ft_basic->gmax,IDENTITY_REMAP,&comp_grid);
        	    front->rect_grid = &comp_grid;

		    /*Reading restart part*/
		...
		else
		{
		    ...
		    /* most function pointers in Front set here */
        	    set_default_front(&Init,front);
		    ...
		    /* FIXME: Why do we have to make interface twice? */
        	    i_intfc(&Init) = front->interf = make_interface(dim);
...
{
	RECT_GRID *gr = front->rect_grid;
	/* Before the pp_clip_front_to_subdomain,
	 * the front->rect_grid is the global grid */
	...
	{
	    clip_front_to_subdomain(front);
	    (void) set_boundary(front->interf,&topological_grid(intfc),
