...
LOCAL   boolean     install_btri_states_from_crx(INTERFACE*,BOND_TRI*,CRXING*,size_t,ORIENTATION);
...
LOCAL   boolean    use_fast_merge_algo = NO;
EXPORT  void    set_fm(boolean flag)
{
	...
	int	 **ips)
{
	...
	DEBUG_ENTER(record_unphysical_ips)
        ...
        merge_tol = 0.1*min3(gr->h[0], gr->h[1], gr->h[2]);
        gmax = gr->gmax;
