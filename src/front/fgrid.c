	...
	INTERFACE	*intfc)
{
	...
	for (i = 0; i < 3; ++i)
	{
        ...
        if(prev_interface(intfc) != NULL)
        {
            ...
            intfc->default_comp = grid_intfc->default_comp;  /* Bug fix for the wrong phase problem of the default_comp */
        }
	...
	if (status == FUNCTION_FAILED)
	{
	    ...
	    //print_interface(intfc);
	    //clean_up(ERROR);
            screen("WARNING in set_grid_intfc_components3d(), "
                   "track_comp_through_crxings3d() failed!\n");
