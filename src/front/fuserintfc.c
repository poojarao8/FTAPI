	...
	double eps)
{
	...
	case DIRICHLET_BOUNDARY:
	    ...
	    return;
    case OPEN_BOUNDARY:             // JAM - Added to handle flowthrough boundry type
	...
	INTERFACE *intfc)	/* intfc may not be the same as front->interf */
{
        ...
        HYPER_SURF         *hs;
        int dim = intfc->dim;
        double  max_curvature = 2.0/front->rect_grid->h[0];
	...
	if (debugging("wlsp_curvature"))
		...
		while (next_point(intfc,&p,&hse,&hs))
		{
		    if (!WLSP_compute_normal3d_cyl(p,hse,hs))
		    {
			...
			p->curvature = f_mean_curvature_at_point3d(p,hse,hs,front);
                    ...
                    if (isnan(p->curvature))
                        p->curvature = 0.0;
                    else if (fabs(p->curvature) > max_curvature)
                        p->curvature = p->curvature/fabs(p->curvature)*max_curvature;
		}
	    }
	    else if (front->coordinate == 'r')
	    {
		next_point(intfc,NULL,NULL,NULL);
		while (next_point(intfc,&p,&hse,&hs))
		    WLSP_compute_normal3d0(p,hse,hs);
