	...
	Front *front)
{
	...
	Front *newfront;
        ...
        if(front->step > 0) //&& front->regrid_restart == NO)  // See note below
        {   
	...
	if (front->grid_intfc != NULL)
        ...
        // This is used to auto-restart a simulation, we don't want that here JAM: 5/9/16
/*
...
*/
        ...
        if(front->step > 0) // && front->regrid_restart == NO) // See note above
	...
	*dt_frac = 0.5; 
	front->dt_frac = dt_frac;
	    ...
	    if (status == MODIFY_TIME_STEP)
            ...
            if (count == 3) // From 9; RK 2/19/2013
                set_redistribution(YES);
/*   // This is related to auto-restarting and we don't want that  JAM - 5/9/16
            if (count == 10)
            {
	...
	double (*get_state)(Locstate))
{
	...
	case 3:
	    ...
	    for (k = 0; k < 2; ++k)
	    {
		    ...
		    blk_cell->var[nv] = grid_array[index];
		    if(debugging("ryan")) 
			printf("index = %d %d %d: %d: state= %f\n",
				ic[0], ic[1], ic[2], index, grid_array[index]);
		    nv++;
	    	}
	    	else
	    	    blk_cell->is_bilinear = NO;
        ...
        F_BASIC_DATA *f_basic)
{
	...
	(void) printf("\n");
        CursorAfterString(infile,"Regrid:");
        for (i = 0; i < f_basic->dim; ++i)
        {
        ...
        CursorAfterString(infile,"Repartition:");
        for (i = 0; i < f_basic->dim; ++i)
        {
