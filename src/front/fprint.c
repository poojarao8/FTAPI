...
LOCAL	void 	gd_plot_var(INTERFACE*,char*,double,char*,double*, /* for 1d */
		double (*get_state_var)(Locstate),int,boolean);
#endif /* defined(__GD__) */
LOCAL   void    xgraph_plot_var(INTERFACE*,char*,double,char*,double*, 
                double (*get_state_var)(Locstate),int); /* for 1d */
...
LOCAL	void	show_front_vtp(Front*,char*,boolean);
	...
	char *out_name)
{
	...
	boolean save_binary_output = is_binary_output();
        ...
        /*if(front->repartition == YES)
        {
        Front           *tempfront;
        RECT_GRID       dual_grid;
        RECT_GRID       *zoom_grid = &(front->new_pp_grid->Zoom_grid);
        char            fname[1024];
        ...
        GL = front->pp_grid->Global_grid.L;
        GU = front->pp_grid->Global_grid.U;

        find_Cartesian_coordinates(pp_mynode(),front->pp_grid,icrds_old);
        for (i = 0; i < dim; ++i)
            ratio[i] = (front->new_pp_grid->gmax[i])/(front->pp_grid->gmax[i]);

        for (i = 0; i < front->new_pp_grid->nn; ++i)
        {
            find_Cartesian_coordinates(i,front->new_pp_grid,icrds_new);
            ...
            if (k)
            {
               //  set the zoom_grid of new_pp_grid here 
                for (j = 0; j < dim; ++j)
                {
                    L[j] = front->new_pp_grid->dom[j][icrds_new[j]];
                    U[j] = front->new_pp_grid->dom[j][icrds_new[j] + 1];
                    ...
                    case 2:
                        lbuf[j] = (icrds_new[j] > 0) ?
                                     front->new_pp_grid->buf[j] : 0;
                        ubuf[j] = (icrds_new[j] <(front->new_pp_grid->gmax[j]-1))
                                   ? front->new_pp_grid->buf[j]:0;
                        break;
                    case 3:
                        lbuf[j] = (icrds_new[j] > 0)
                                ? front->new_pp_grid->buf[j] :
                ...
                set_rect_grid(L,U,GL,GU,lbuf,ubuf,gmax,dim,
                    &front->rect_grid->Remap,&front->new_pp_grid->Zoom_grid);
                ...
                for (j = 0; j < dim; ++j)
                {
                    if (icrds_new[j] > 0)
                        rect_boundary_type(tempfront->interf,j,0)
                                = SUBDOMAIN_BOUNDARY;
                    if (icrds_new[j] <(front->new_pp_grid->gmax[j]-1))
...
//                }
                ...
                sprintf(intfc_name,"%s/intfc-t%s",dirname,right_flush(step,7));

                if (numnodes > 1)
                    sprintf(intfc_name,"%s-p%s",intfc_name,right_flush(i,4));

                out_file = fopen(intfc_name,"w");
                print_title(out_file,"");
                fprint_front_time_stamp(out_file,tempfront);
                fprintf(out_file,"\n#");
                fprint_interface(out_file,tempfront->interf);
                fclose(out_file);
	    ...
	    if (first)
	    {
		if (pp_mynode() == 0 && !create_directory(out_name,NO))
		{
		    screen("Cannot create directory %s\n",out_name);
		    clean_up(ERROR);
		}
	...
	sprintf(intfc_name,"%s",out_name); ///intfc-ts%s",out_name,right_flush(step,7));
	if(pp_numnodes() > 1)
	   sprintf(intfc_name,"%s-p%s",intfc_name,right_flush(pp_mynode(),4));

//	if (pp_mynode() == 0 && !create_directory(dirname,YES))
//	{
//	    screen("Cannot create directory %s\n",dirname);
//	    clean_up(ERROR);
//        }
	...
	boolean first)
{
	...
	pwidth = global_gr->gmax[0];
	pheight = global_gr->gmax[1];
	debug_print("HDF","Entered hdf_plot_var2d().\n");
	...
	debug_print("HDF","Left hdf_plot_var2d().\n");
}	/* end hdf_plot_var2d */
	...
	Front *front)
{
	...
	for (i = 0; i < num_var; ++i)
	{
	    gd_plot_var(front->grid_intfc,dirname,front->time,
			hdf_movie_var->var_name[i],hdf_movie_var->top_var[i],
			hdf_movie_var->get_state_var[i],i,first);
	}
	first = NO;
}	/* end FrontGDMovie1d */
        ...
        boolean first)
{
	...
	if (first)
	{
	    ...
	    xmin = L[0];	xmax = U[0];
	    cmin[ivar] -= 0.15*height;    cmax[ivar] += 0.30*height;
	}
