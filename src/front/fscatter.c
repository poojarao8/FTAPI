	...
	Front		*front)
{
	...
	status = form_subintfc_via_communication(front);  /* Main Function in scatter */
	...
	int   rbt[3][2])
{
	...
	if(dim == 3)
	{
	    ...
	    /* Setting the topological grid to be expanded dual grid */
	...
	Front		*front)
{
	...
	/* If Gmax % Pmax != 0, it will fill in the pp_grid with extra 
	 * in the first several subdomains. E.g. Gmax = 15, Pmax = 5, 
	 * then the partition would be, 3, 3, 3, 2, 2 */
	...
	for (i = 0; i < dim; ++i)
	{
	    ...
	    /* irint is the rounding down and up of a double value */
		...
		/* lbuf and ubuf equals to 4 in 3D case */
	    	break;
	    }
	}
	...
	if (debugging("set_pp_grid"))
	{
        ...
        /*front->repartition = NO;
        for (i = 0; i < dim; ++i)
        {
            if (front->gmax_new_pp_grid[i] != front->pp_grid->gmax[i])
                front->repartition = YES;
        }
        //if(front->repartition == YES)
        {
            static PP_GRID new_Pp_grid;
            front->new_pp_grid = &new_Pp_grid;
            new_Pp_grid.gmax[0] = front->gmax_new_pp_grid[0];
            new_Pp_grid.gmax[1] = front->gmax_new_pp_grid[1];
            new_Pp_grid.gmax[2] = front->gmax_new_pp_grid[2];
            copy_rect_grid(&new_Pp_grid.Global_grid,&pp_grid->Global_grid);

                //Setting up final output partition 
                front->new_pp_grid->nn = 1;
                for (i = 0; i < dim; ++i)
                {
                    int new_Gmax, new_Pmax, new_k;
                    int new_basic_slices, new_extra_slices;

                    front->new_pp_grid->buf[i] = buffer_zones(init)[i];
                    new_Pmax = front->new_pp_grid->gmax[i];
                    front->new_pp_grid->nn *= new_Pmax;

                    uni_array(&front->new_pp_grid->dom[i],new_Pmax + 1,FLOAT);

                    front->new_pp_grid->dom[i][0]    = pp_grid->Global_grid.L[i];
                    front->new_pp_grid->dom[i][new_Pmax] = pp_grid->Global_grid.U[i];
                    new_Gmax = pp_grid->Global_grid.gmax[i];

                    new_basic_slices = new_Gmax / new_Pmax;
                    new_extra_slices = new_Gmax % new_Pmax;

                    for (new_k = 1; new_k < new_Pmax; ++new_k)
                    {
                        if (new_k < new_extra_slices)
                            front->new_pp_grid->dom[i][new_k] = new_k*(new_basic_slices
                                + 1)*h[i] + front->new_pp_grid->dom[i][0];
                        else
                            front->new_pp_grid->dom[i][new_k] = (new_k*new_basic_slices +
                                new_extra_slices)*h[i]
                                + front->new_pp_grid->dom[i][0];
                    }
                }
        }*/
