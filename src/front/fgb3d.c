	...
	Front     *front)
{
	...
	make_interface_topology_lists(intfc); /* make_tri_comp_lists */
...
LOCAL   boolean    increased_recon_box = NO;
EXPORT  void    set_increased_recon_box(boolean flag)
{
        increased_recon_box = flag;
}
	...
	RECT_BOX	**boxes)
{
	...
	while (i < num_ip)
	{
        ...
        if(increased_recon_box == YES)
        {
            for (k = 0; k < 3; k++)
            {
                box->bmin[k] = box->bmin[k] - 2;
                box->bmax[k] = box->bmax[k] + 2;
            }
        }
