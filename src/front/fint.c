	...
	INTERFACE	*intfc)
{
	...
	dim = intfc->dim;
	fuh = f_user_hook(dim);
	f_user_interface(intfc) = *fuh;/* F_USER_INTERFACE related to intfc here */
	size_of_state(intfc) = size_of_intfc_state;
	set_size_of_intfc_state(0);/* reset the global variable to 0 */
	...
	HYPER_SURF	*old_hs)
{
	...
	if (wave_type(old_hs) == MOVABLE_BODY_BOUNDARY)
	{
	    int i;
	...
	NODE		*end)
{
	...
	if(intfc->dim==2)
	    Hyper_surf(new_curve)->surf_type = Hyper_surf(curve)->surf_type;
	...
	boolean copy_tris)
{
	...
	Hyper_surf(news)->surf_type = Hyper_surf(s)->surf_type;
