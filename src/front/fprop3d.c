        ...
        double              *V)
{
        ...
        // JAM - we want to check to make sure this is a point we want to propagate
        // if not i.e. if it is a boundary node, it should be set to the current point
        if ((fabs(Coords(oldp)[0] - front->rect_grid->VL[0]) < tol) || 
            (fabs(Coords(oldp)[0] - front->rect_grid->VU[0]) < tol) ||
            ( (dim > 1) &&
             ((fabs(Coords(oldp)[1] - front->rect_grid->VL[1]) < tol) || 
              (fabs(Coords(oldp)[1] - front->rect_grid->VU[1]) < tol)) 
            )      ||
            ( (dim > 2) &&
              ((fabs(Coords(oldp)[2] - front->rect_grid->VL[2]) < tol) ||
               (fabs(Coords(oldp)[2] - front->rect_grid->VU[2]) < tol))
            )
           )
        {
            vel[0] = vel[1] = vel[2] = 0.0;
            vm1[0] = vm1[1] = vm1[2] = 0.0;
        }
        else
        {
            (*front->vfunc)(front->vparams,front,oldp,oldhse,oldhs,vel);
            
            // JAM - Probably should just change it to use 1st order point propagate
            //       but I have added some stuff to this function so just commenting
            //       out here.  2nd order is useless and actually incorrect as after
            //       the front moves there is no update to the components so the only
            //       thing that can happen is that moves across a cell center and then
            //       gets two cells which are the same material which is weighted as 
            //       1/2 of its velocity.
            //for (i = 0; i < dim; ++i)
        ...
        for (i = 0; i < dim; ++i)
        {
            // JAM - Commented out for reason above
	        //V[i] = 0.5*(vel[i] + vm1[i]);
	        V[i] = vel[i];
        ...
        //filename = fopen("frontMove.txt","a");
        //fprintf(filename, "%d %18.16e %18.16e (%18.16e %18.16e) : (%18.16e %18.16e) -> (%18.16e %18.16e)\n",front->step,front->time,dt,V[0],V[1],Coords(oldp)[0],Coords(oldp)[1],Coords(newp)[0],Coords(newp)[1]);
        //fclose(filename);        
