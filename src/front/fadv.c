	...
	Front		*front)
{
	...
	case 3:
	    front->_pre_advance_front = NULL;
	    front->_advance_front = advance_front3d_tracking_control;/* locally grid base here */
	    if (efss == YES)
	...
	INTERFACE	*intfc)
{
	...
	new_intfc = copy_interface(intfc); /* f_copy_interface called */
	...
	if (pp_min_status(stat) == NO)
	{
	    if (stat == YES)
	    {
	        ...
	        delete_status = (delete_interface(new_intfc)) ? YES : NO;/* f_delete_interface called */
		if (delete_status == NO)
		{
	...
	POINTER  wave)
{
	...
	// newfront is a literal binary copy of front
	*newfront = copy_front(front);
	Interface_redistributed(*newfront) = NO;
...
begin_advance_front2d:
	...
	// sets next_node and prev_node on all nodes in interf
	// so that they exist in a linked list
	// also sets the head of the list to first_node(front)
	// and the tail of the list to last_node(front)
...
sync_prop_stat1:
    ...
    // JAM : Added 4/4/2016 
    // It seems to me that when we aren't redistributing we still want to delete short bonds
    // These can be created when running in parallel because it forces a point at the proc
    // cut and if this moves slightly off a new point is forced at the proc cut.
    // So when we aren't redistributing, we should still remove short bonds.
...
LOCAL   boolean    use_redistribution = NO;
EXPORT  void    set_redistribution(boolean flag)
{
        use_redistribution = flag;
}

LOCAL   boolean    use_gb = NO;
EXPORT  void    set_gb(boolean flag)
{
        use_gb = flag;
}
...
LOCAL int mixed_advance_front3d(
	...
	POINTER		wave)
{
    	start_clock("mixed_advance_front3d");
	int status;
	double   dt_frac_restore = *dt_frac;
	DEBUG_ENTER(mixed_advance_front3d)

        if(front->use_fast_merge_algo == YES)
            set_fm(YES);
        else
            set_fm(NO);
	...
	//			    front,newfront,wave);
        ...
        if(use_gb == YES)
        {
            printf("ERROR: call global grid based reconstruction.\n");
            clean_up(ERROR); 

            // *dt_frac = dt_frac_restore;
            front->redis_flag = NO;

            status = reconstruct_front_advance_front3d(dt,dt_frac,front,
                                                           newfront,wave);
            //new avoid grid based
            //distort_all_points(*newfront);
            //smooth_redistribute(*newfront);
        }
        else
        {
            status = preserve_front_advance_front3d(dt,dt_frac,
	...
	DEBUG_LEAVE(mixed_advance_front3d)
    	stop_clock("mixed_advance_front3d");
	return status;
...
LOCAL int preserve_front_advance_front3d(
	...
	POINTER		wave)
{
    	start_clock("preserve_front_advance_front3d");
	...
	if (front->step % Frequency_of_redistribution(front,GENERAL_WAVE) == 0 && front->dt > 1e-5)
	    do_redist = YES;

        set_cut_ref(NO);
        if(use_redistribution == YES)
        {
	    start_clock("Redistribution_Before_Global_Recon");
            fprintf(stdout, "Redistribution call to avoid a global reconstruction\n");
            ...
            switch (redistribute(tempfront,YES,NO))
            {
            case GOOD_REDISTRIBUTION:
                Redistribution_count(tempfront) = Redistribution_count(tempfront);
                status = GOOD_STEP;
                break;

            case UNABLE_TO_UNTANGLE:
                *dt_frac *= TIME_STEP_REDUCTION_FACTOR(tempfront->interf);
                status = MODIFY_TIME_STEP;
            ...
            case BAD_REDISTRIBUTION:
                *dt_frac = Min_time_step_modification_factor(tempfront);
                status = MODIFY_TIME_STEP;
            ...
            default:
                ...
                debug_front("ERROR_front","after error",tempfront);
                clean_up(ERROR);
                break;
            }
            //set_use_bd_dist(NO);
            ...
            if (status != GOOD_STEP)
            {
                ...
                (void) printf("WARNING in use_redistribution in preserve_front_advance_front3d(), "
	    ...
	    stop_clock("Redistribution_Before_Global_Recon");
        }

	*newfront = copy_front(front);/* f_copy_front called */
	(*newfront)->redis_flag = front->redis_flag;
	Interface_redistributed(*newfront) = NO;
	has_tracked_surfaces = (front->interf->surfaces != NULL) ? YES : NO;
	if (pp_max_status(has_tracked_surfaces) == NO)/* If there's no surface there for each processor */
	{
	    set_size_of_intfc_state(size_of_state(front->interf));
	    set_copy_intfc_states(YES);
	    (*newfront)->interf = pp_copy_interface(front->interf);
	...
	if(status == REPEAT_TIME_STEP)
	{
    	...
    	stop_clock("preserve_front_advance_front3d");
	...
	boolean		reconstruct_front)
{
	...
	if (front->_point_propagate != NULL)
	{
	    ...
	    if (front->curve_propagate != NULL) /* curve_propagate is NULL in 3D */
	        propagate_curve_points(front,newfront,wave,dt);
	
	    if (front->node_propagate != NULL)/* node_propagate is NULL in 3D */
	    {
	...
	stop_clock("scatter_front");
        ...
        init_intfc_curvature3d(newfront,newfront->interf);/* Calculate the curvature */
	if (front->_tan_point_propagate != NULL) /* Not allowed in 3D */
	{
        ...
        double           *V)
{
	...
	(void) next_point(intfc_old,NULL,NULL,NULL);
	    ...
	    if(Boundary_point(newp))
	     	continue;
	    point_propagate(front,wave,oldp,newp,oldhse,oldhs,dt,V); /* ifluid_point_propagate called */
	}
