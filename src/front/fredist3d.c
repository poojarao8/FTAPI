	...
	INTERFACE	*intfc)
{
	...
	set_increased_buffer_grid(comp_grid, &c_gr, 5, intfc);
	...
	boolean		restart_init)
{
	...
	if (Tracking_algorithm(fr) == LOCALLY_GRID_BASED_TRACKING)
	{
	    ...
	    //status = small_area_tri_on_intfc(fr->interf);
            //pp_gsync();
	    //if(!pp_min_status(status) || fr->step == 3196)
	...
	Front		*fr)
{
	...
	for (tri=first_tri(s); !at_end_of_tri_list(tri,s); tri=tri->next)
	{
        ...
        //printf("\nNumber of tris in redistribution : %d %d %d %d\n", nt,nf,nl,ns);
	...
	Front		*fr)
{
	...
	for (tri=first_tri(s); !at_end_of_tri_list(tri,s); tri=tri->next)
	{
		...
		*/
        ...
        //printf("\nNumber of small tris in redistribution : %d %d %d %d\n", nt,nf,nl,ns);
	...
	Front		*fr)
{
	...
	for (tri=first_tri(s); !at_end_of_tri_list(tri,s); tri=tri->next)
	{
        ...
        //printf("\nNumber of bad tris in redistribution : %d %d %d %d\n", nt,nf,nl,ns);
