...
IMPORT boolean WLSP_compute_normal3d_cyl(POINT*,HYPER_SURF_ELEMENT*,HYPER_SURF*);
...
IMPORT	void	vtk_interface_plot(const char*,INTERFACE*,boolean,double,int,char coordinate);
IMPORT void vtk_interface_plot_byname(const char *name, INTERFACE  *intfc,
	boolean print_in_binary, double time, int step, char coordinate);

IMPORT	void	vtp_interface_plot(const char*,INTERFACE*,boolean,double,int,char coordinate);
...
IMPORT  void    gd_2d_intfc(char*,char*,INTERFACE*,RECT_GRID*,int);
#endif /* defined __GD__ */
IMPORT	char	*get_vtk_file_name(char*,const char*,const char*,size_t*);
IMPORT	char	*get_vtp_file_name(char*,const char*,const char*,size_t*);
...
/* IMPORT	boolean	redistribute_surface(SURFACE*,RECT_GRID*,TRI_REDIST_PARAMS); */
IMPORT	boolean	surface_needs_redist(SURFACE*,RECT_GRID*,TRI_REDIST_PARAMS);
