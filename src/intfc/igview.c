...
#ifdef USE_CGNS
EXPORT  char   *get_cgns_file_name(char*,const char*,const char*,size_t*);
LOCAL   void    cgns_print_box(const char*,const double*,const double*,boolean);
LOCAL   void    cgns_plot_surfaces(INTERFACE*,const double*,
                                  const double*,boolean,
                                  const char*,const char*,
...
EXPORT  char   *get_vtp_file_name(char*,const char*,const char*,size_t*);
LOCAL   void    vtk_print_box(const char*,const double*,const double*,boolean);
LOCAL   void    vtp_plot_surfaces(INTERFACE*,const double*,
                                  const double*,boolean,
                                  const char*,const char*,
...
LOCAL   void    vtk_plot_surfaces(INTERFACE*,const double*,
                                  ...
                                  const char*,const char*,
                                  boolean,SURFACE_COLOR,SURFACE_COLOR,boolean,double,int,char);
LOCAL void vtk_plot_surfaces_byname( INTERFACE *intfc, const double *BBL, 
		const double *BBU, boolean clip, const char *name, 
		boolean bdry, SURFACE_COLOR color1, SURFACE_COLOR color2, 
		boolean print_in_binary, double time, int step, 
		char coordinate);

LOCAL   void    vtk_plot_curves( INTERFACE*, const double*, 
                                 const double*, const char*, 
                                 const char*,SURFACE_COLOR,int,boolean,double,int);
LOCAL void vtk_plot_curves_byname( INTERFACE *intfc, const double *BBL, 
				const double *BBU, const char *name, 
				SURFACE_COLOR color, int width, 
				boolean  print_in_binary, double time, 
				int step);
	...
	size_t     *fname_len)
{
	...
	if (strlen(dname) != 0)
	    (void) sprintf(fname,"%s/%s.vtp",dname,name);
	else
	    (void) sprintf(fname,"%s.vtp",name);
	return fname;
...
EXPORT void vtp_interface_plot(
	const char *dname,
	INTERFACE  *intfc,
	boolean print_in_binary,
	double time,
...
*/
	if (intfc->dim == 3)
	{
            ...
            vtp_plot_surfaces(intfc,BBL,BBU,NO,dname,name,NO,pRED,pRED,
                              print_in_binary,time,step,coordinate);
	}

}		/*end vtp_interface_plot*/
	...
	char          coordinate)
{
	...
	fname = get_vtp_file_name(fname,dname,name,&fname_len);
	...
	if (hardware_is_little_endian())
	    ...
	    fprintf(file, "<VTKFile type=\"PolyData\" version=\"0.1\" byte_order=\"BigEndian\">\n");
	fprintf(file, "  <PolyData>\n");
	fprintf(file, "    <Piece NumberOfPoints=\"%d\" NumberOfVerts=\"0\" NumberOfLines=\"0\" NumberOfStrips=\"0\" NumberOfPolys=\"%d\">\n", npts, ntris);

	fprintf(file, "      <Points>\n");

	if(print_in_binary)
	{
	    fprintf(file, "        <DataArray type=\"Float64\" NumberOfComponents=\"3\" format=\"binary\">\n");
	    for (i = 0; i < npts; ++i)
	    {
		double vals[1];
		for(j = 0; j < 3; ++j)
	...
	else
	{
	    ...
	    for (j = 0; j < ntris; ++j)
	        (void) fprintf(file,"%d %d %d\n", verts[4*j],verts[4*j+1],verts[4*j+2]);
	}
	fprintf(file, "        </DataArray>\n");
	if(print_in_binary)
	{
	...
	if(print_in_binary)
	{
	    fprintf(file, "        <DataArray type=\"Float64\" Name=\"normal\" NumberOfComponents=\"3\" format=\"binary\">\n");
		...
		for(j = 0; j < 3; ++j)
		{
		    ...
		    fwrite(nor_vals, sizeof(double), 1, file);
		}
	    }
	    fprintf(file, "\n");
	}
	...
	{
	    fprintf(file, "        <DataArray type=\"Float64\" Name=\"normal\" NumberOfComponents=\"3\" format=\"ascii\">\n");
	...
	if(print_in_binary)
	{
	    fprintf(file, "        <DataArray type=\"Float64\" Name=\"curvature\" NumberOfComponents=\"1\" format=\"binary\">\n");

	    double cur_vals[1];
	    for(i = 0; i < npts; ++i)
	    {
		cur_vals[0] = pts_curvature[i];
		fwrite(cur_vals, sizeof(double), 1, file);
	    }
	    fprintf(file, "\n");
	}
	else
	{
	    fprintf(file, "        <DataArray type=\"Float64\" Name=\"curvature\" NumberOfComponents=\"1\" format=\"ascii\">\n");
	    for(i = 0; i < npts; ++i)
		fprintf(file,"%-9g\n",pts_curvature[i]);
	}
	fprintf(file, "        </DataArray>\n");
	...
	)
{
	...
	/* boundary vertex size (zero if elements not sorted)*/
   	isize[2]=0;
	/* create zone*/
   	cg_zone_write(index_file,index_base,zonename,isize,Unstructured,
		&index_zone);
	...
	/* index no of first element*/
   	nelem_start=1;
	    ...
	    ielem[0][0] = 1; ielem[0][1] = 2;
	    ielem[1][0] = 2; ielem[1][1] = 3;
	    ielem[2][0] = 3; ielem[2][1] = 4;
	    ielem[3][0] = 4; ielem[3][1] = 1;
	    ielem[4][0] = 5; ielem[4][1] = 6;
	    ielem[5][0] = 6; ielem[5][1] = 7;
	    ielem[6][0] = 7; ielem[6][1] = 8;
	    ielem[7][0] = 8; ielem[7][1] = 5;
	    ielem[8][0] = 1; ielem[8][1] = 5;
	    ielem[9][0] = 2; ielem[9][1] = 6;
	    ielem[10][0]= 3; ielem[10][1]= 7;
	    ielem[11][0]= 4; ielem[11][1]= 8;

	/* index no of last element */
   	nelem_end = 12;
	/* unsorted boundary elements*/
   	nbdyelem=0;
 	/* write BAR_2 element connectivity (user can give any name)*/
   	cg_section_write(index_file,index_base,index_zone,"Elem",BAR_2,
		nelem_start,nelem_end,nbdyelem,*ielem,&index_section);
	/* close CGNS file*/
   	cg_close(index_file);
}		/*****end cgns_print_box*******/

EXPORT	char   *get_cgns_file_name(
	char       *fname,
	const char *dname,
	const char *name,
	size_t     *fname_len)
{
	...
	if (strlen(dname) != 0)
	    (void) sprintf(fname,"%s/%s.cgns",dname,name);
	else
	    (void) sprintf(fname,"%s.cgns",name);
	return fname;
}		/*end get_cgns_file_name*/

EXPORT void cgns_interface_plot(
	const char *dname,
	INTERFACE  *intfc,
	boolean print_in_binary,
	double time,
	int step)
{
	...
	if (create_directory(dname,YES) == FUNCTION_FAILED)
	{
	    (void) printf("WARNING in cgns_interface_plot(), directory "
			  "%s doesn't exist and can't be created\n",dname);
	    return;
	}
	if (intfc->dim == 2)
...
}		/*end cgns_interface_plot*/


LOCAL	void	cgns_plot_surfaces(
	INTERFACE     *intfc,
	const double   *BBL,
	const double   *BBU,
	boolean          clip,
	...
	int 	      step)
{
	...
	RECT_GRID *gr = &topological_grid(intfc);
        /************* New for CGNS *******************************/
	static double     *xs = NULL, *ys = NULL, *zs = NULL;
	static int 	  **ielem = NULL;
	int 		  num_pts;
   	int isize[3];
   	int index_file,icelldim,iphysdim,index_base;
   	int index_zone,index_coord,index_section;  
	...
	if (alloc_len_verts < 3*num_tris)
	{
	    if (verts != NULL)
		free(verts);
	    alloc_len_verts = 3*num_tris;
	    uni_array(&verts,alloc_len_verts,INT);
	    bi_array(&ielem,num_tris,3,INT);
	}
	for (i = 0; i < 3; i++)
	{
	    L[i] = gr->L[i] - 0.5*gr->h[i]; 
        ...
        for (npts=0, ntris=0, num_surfs=0, s = intfc->surfaces; s && *s; ++s)
	{
		...
		plot_surf = NO;
			...
			ielem[ntris][k] = Index_of_point(p)+1;
			}
			++ntris;
		    }
		}
	...
	/* unsorted boundary elements*/
   	nbdyelem=0;
 	/* write TRI_3 element connectivity (user can give any name)*/
   	cg_section_write(index_file,index_base,index_zone,"Elem",TRI_3,
		nelem_start,nelem_end,nbdyelem,*ielem,&index_section);
	/* close CGNS file*/
   	cg_close(index_file);
}	/*end cgns_plot_surfaces*/


LOCAL   void    cgns_plot_curves(
        INTERFACE     *intfc,
	const double   *BBL,
	const double   *BBU,
        const char    *dname,
...
EXPORT void vtk_interface_plot_byname(
	const char *name,
	INTERFACE  *intfc,
	boolean print_in_binary,
	double time,
	...
	char coordinate)
{
	...
	double *BBU = topological_grid(intfc).GU;
        ...
        if(strcmp(&(name[length-4]), ".vtk")==0)
            length-=4;
        strncpy(filename, name, length);
        filename[length] = '\0';
        if(pp_numnodes()>1)
            strcat(strcat(filename, "-nd"), right_flush(pp_mynode(),4));
        strcat(filename, ".vtk");

	if (intfc->dim == 2)
	    vtk_plot_curves_byname(intfc, BBL, BBU, filename, pRED, 10, 
				print_in_binary, time, step);

	if (intfc->dim == 3)
	{
	    vtk_plot_surfaces_byname(intfc, BBL, BBU, NO, filename, NO, pRED, 
	    		pRED, print_in_binary, time, step, coordinate);
	}

}		/*end vtk_interface_plot*/

LOCAL   void    vtk_plot_curves_byname(
        INTERFACE     *intfc,
	const double   *BBL,
	const double   *BBU,
        const char    *name,
	...
	int 	      step)
{
	...
	{	
	    ...
	    // Plot normals
	    (void) fprintf(file, "POINT_DATA %d\n",tot_num_pnts);
	    (void) fprintf(file, "NORMALS normal double\n");
	    {
		HYPER_SURF *hs;
		POINT *p;
...
LOCAL	void	vtk_plot_surfaces_byname(
	INTERFACE     *intfc,
	const double  *BBL,
	const double  *BBU,
	boolean       clip,
