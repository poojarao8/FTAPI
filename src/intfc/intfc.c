	...
	int		dim)
{
	...
	if (new_table(dim) == NULL)/* storage allocated here */
	    return NULL;
	cur_intfc->dim = dim;
	...
	user_make_interface(cur_intfc); /* call f_user_make_interface */
	...
	INTERFACE	*intfc)
{
	...
	interface_reconstructed(new_intfc) = interface_reconstructed(intfc);
        prev_interface(new_intfc) = prev_interface(intfc);  /* Copy the previous interface */
...
//	assert(curves[0]->hs->velocity_func == curve->hs->velocity_func);
//	assert(curves[0]->hs->velocity_params == curve->hs->velocity_params);
//	assert(curves[1]->hs->velocity_func == curve->hs->velocity_func);
//	assert(curves[1]->hs->velocity_params == curve->hs->velocity_params);
	assert(curves[0]->hs->surf_type == curve->hs->surf_type);
	assert(curves[1]->hs->surf_type == curve->hs->surf_type);
	...
	BOND		**bond_at_join)
{
	...
	//assert(curve1->hs->velocity_func == curve2->hs->velocity_func);
	//assert(curve1->hs->velocity_params == curve2->hs->velocity_params);
	//curve->hs->velocity_func = curve1->hs->velocity_func;
	//curve->hs->velocity_params = curve1->hs->velocity_params;
	assert(curve1->hs->surf_type == curve2->hs->surf_type);
	curve->hs->surf_type = curve1->hs->surf_type;
	...
	int		dim)
{
	...
	cur_IT->remainder = 0;
	cur_IT->num_chunks = 0;
	cur_IT->_ChunkSize = uh->_ChunkSize;/* 50000*double in 3D */
	cur_IT->top = NULL;
	cur_intfc = NULL;
